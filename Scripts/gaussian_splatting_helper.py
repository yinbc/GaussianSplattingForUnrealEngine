...
class GaussianSplattingHelper:
    ...
    def readCameraIntrinsics(self):
        """Read camera intrinsics from camera_intrinsics.txt generated by UE5"""
        intrinsics_path = "./camera_intrinsics.txt"
        if os.path.exists(intrinsics_path):
            with open(intrinsics_path, 'r') as f:
                lines = f.readlines()
                if len(lines) >= 3:
                    camera_model = lines[0].strip()
    ...
    def executeSparseReconstruction(self):
        ...
        # Read camera intrinsics from UE5
        camera_intrinsics = self.readCameraIntrinsics()

        command = f"{self.args.colmap} feature_extractor --database_path ./database.db --image_path ./images"
        if camera_intrinsics:
            printImmediately(f"Using camera intrinsics from UE5: model={camera_intrinsics['model']}, params={camera_intrinsics['params']}")
            command += f" --ImageReader.camera_model {camera_intrinsics['model']}"
            command += f" --ImageReader.camera_params \"{camera_intrinsics['params']}\""
            command += " --ImageReader.single_camera 1"
        else:
            printImmediately("Warning: camera_intrinsics.txt not found, using default SIMPLE_PINHOLE with auto estimation")
            command += " --ImageReader.camera_model SIMPLE_PINHOLE"
        ...
        # If we have known camera intrinsics from UE5, don't let COLMAP refine them
        if camera_intrinsics:
            command += " --Mapper.ba_refine_focal_length 0 --Mapper.ba_refine_principal_point 0 --Mapper.ba_refine_extra_params 0"
            printImmediately("Camera intrinsics will be fixed during bundle adjustment")
        if self.args.mapper:
